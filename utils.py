...
def collate_fn(batch):
    ...
    gpt_labels = [f["gpt_labels"] for f in batch]
    ...
    gpt_labels = [torch.tensor(label) for label in gpt_labels]
    gpt_labels = torch.cat(gpt_labels, dim=0)
    ...
    output = (input_ids, input_mask, labels, entity_pos, hts, sent_pos, sent_labels_tensor, attns, gpt_labels)
