...
def load_input(batch, device, tag="dev"):
    ...
    input = {'input_ids': batch[0].to(device),
            ...
            'tag': tag,
            'entail_scores': batch[-1].to(device)
            } 

    return input

def train(args, model, train_features, dev_features, pruner=None):
        ...
        for epoch in tqdm(train_iterator, desc='Train epoch'):
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) == len(train_dataloader) or (args.evaluation_steps > 0 and num_steps % args.evaluation_steps == 0 and step % args.gradient_accumulation_steps == 0):
                    ...
                    dev_scores, dev_output, official_results, results = evaluate(args, model, dev_features, tag="dev", pruner=pruner)
...
def evaluate(args, model, features, tag="dev", pruner=None):
    ...
    scores = {"dev_pre": best_re[0] * 100, "dev_recall": best_re[1] * 100, "dev_F1": best_re[-1] * 100, "dev_evi_F1": best_evi[-1] * 100, "dev_F1_ign": best_re_ign[-1] * 100,"dev_pre_ign": best_re_ign[0] * 100,"dev_recall_ign": best_re_ign[1] * 100}
...
def main():
    ...
    if args.do_train:  # Training
        ...
        print("Test scores of best ckpt..........................")
        print(test_scores)
        headers = ["precision", "recall", "F1"]
